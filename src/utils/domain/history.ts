...
import { generateId, formatAngle } from './bearing';

export const MAX_RECORDS = 2000;

export const KEY_NODE_TYPES: OperationType[] = [
  'axis_save',
  'plan_switch',
  'plan_create',
  'plan_delete',
...
export function detectAnomaly(record: OperationRecord): OperationRecord {
...
export function calculateStatistics(records: OperationRecord[]) {
...
export function isKeyNodeType(type: OperationType): boolean {
  return KEY_NODE_TYPES.includes(type);
}
